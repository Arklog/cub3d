...
/*   Updated: 2024/03/14 17:36:33 by laliao           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Assigning blindspot value if the angle is close to 0 or 180 because there
won't be a horizontal wall intersection.

(M_PI / 180) : conversion from degree to radians*/

t_point	ft_horizontal_inter(t_game *game, double al, double ray_angle)
{
	t_point	a;
	t_point	diff;
	...
	if ((ray_angle >= 90 && ray_angle <= 180) || ray_angle >= 270)
		a.x = game->p1.x + (a.y - game->p1.y) / tan(al * (double)(M_PI / 180));
	else
		a.x = game->p1.x + (game->p1.y - a.y) / tan(al * (double)(M_PI / 180));
	if (!(a.x / TILE > 0 && a.x / TILE < game->map_lenght))
		return (game->blindspot);
	diff.x = TILE / tan(al * (double)(M_PI / 180));
...
t_point	ft_vertical_inter(t_game *game, double al, double ray_angle)
{
	t_point	b;
	t_point	diff;
	...
	if ((ray_angle >= 90 && ray_angle <= 180) || ray_angle >= 270)
		b.y = game->p1.y + (b.x - game->p1.x) * tan(al * (double)(M_PI / 180));
	else
		b.y = game->p1.y + (game->p1.x - b.x) * tan(al * (double)(M_PI / 180));
	if (!(b.y / TILE > 0 && b.y / TILE < game->map_height))
		return (game->blindspot);
	diff.y = TILE * tan(al * (double)(M_PI / 180));
	if (ray_angle <= 180)
		diff.y = -diff.y;
...
/* each call of the function will create a new view of the whole screen
dimension from player position */
